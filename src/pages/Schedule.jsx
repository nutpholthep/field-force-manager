...
import { ChevronLeft, ChevronRight, CalendarDays, GripVertical, Calendar, ChevronDown, ChevronRight as ChevronRightIcon } from "lucide-react";
...
function hasOverlap(existingJobs, newJob) {
  ...
  return (
    <div
      ...
      className={`rounded px-1.5 py-1 text-[10px] leading-tight border select-none transition-shadow ${
        ...
        wo.priority === "critical" ? "bg-red-50 border-red-200 text-red-700" :
        wo.priority === "high"     ? "bg-orange-50 border-orange-200 text-orange-700" :
        wo.priority === "medium"   ? "bg-blue-50 border-blue-200 text-blue-700" :
                                     "bg-slate-50 border-slate-200 text-slate-600"
      }`}
    >
      <div className="flex items-start gap-1">
        <div {...prov.dragHandleProps} className="mt-0.5 cursor-grab active:cursor-grabbing shrink-0">
...
// ── Week View ────────────────────────────────────────────────────────────────
// Groups rows by Role, each role is a collapsible section header,
// then lists technicians in that role below.
function WeekView({ groupedData, days, workOrders, technicians, queryClient, isToday }) {
  const [collapsedRoles, setCollapsedRoles] = useState({});

  const toggleRole = (role) => setCollapsedRoles(prev => ({ ...prev, [role]: !prev[role] }));
  ...
  const onDragEnd = async (result) => {
    ...
    const destJobs = workOrders.filter(
      w => w.assigned_technician_id === destTechId && w.scheduled_date === destDate && w.id !== wo.id
    );
    if (hasOverlap(destJobs, wo)) {
      toast.error(`⚠️ Time conflict — ${destTech.full_name} มีงานทับซ้อนกันในเวลา ${wo.scheduled_time || "นี้"} วันที่ ${destDate}`);
      return;
    }
            ...
            <tbody>
              {groupedData.length === 0 ? (
                ...
                groupedData.map(({ role, roleLabel, techs }) => {
                  ...
                  const roleCls = roleColors[role] || "bg-slate-50 text-slate-600 border-slate-200";
                  return (
                    <React.Fragment key={role}>
                      ...
                      >
                        <td className="px-3 py-2 border-r border-slate-200">
                          <div className="flex items-center gap-2">
                            ...
                            <span className={`px-2 py-0.5 rounded-md text-[11px] font-semibold border ${roleCls}`}>
                              {roleLabel}
                            </span>
                            <span className="text-[11px] text-slate-400">{techs.length} คน</span>
                          </div>
                        </td>
                        {/* Summary: total jobs per day for this role */}
                        {days.map(day => {
                          const dateStr = day.format("YYYY-MM-DD");
                          const total = techs.reduce((sum, { dayJobs }) => sum + (dayJobs[dateStr]?.length || 0), 0);
                          return (
                            <td key={dateStr} className={`border-r border-slate-200 text-center ${isToday(day) ? "bg-blue-50/30" : ""}`}>
                              {total > 0
                                ? <span className="inline-flex items-center justify-center w-5 h-5 rounded-full bg-slate-300 text-slate-700 text-[10px] font-bold">{total}</span>
                      ...
                      {/* Technician rows */}
                      {!isCollapsed && techs.map(({ tech, dayJobs }) => (
                        <tr key={tech.id} className="border-b border-slate-100 hover:bg-slate-50/30">
                          <td className="p-2 pl-8 border-r border-slate-200">
                            <div className="flex items-center gap-2">
                              ...
                              <div>
                                <p className="text-xs font-medium text-slate-800 leading-tight">{tech.full_name}</p>
                                <p className="text-[10px] text-slate-400">{tech.zone_name || "No zone"}</p>
                              </div>
                            </div>
                          </td>
                          {days.map(day => {
                            ...
                            return (
                              <td key={dateStr} className={`p-1 border-r border-slate-200 align-top ${isToday(day) ? "bg-blue-50/30" : ""}`}>
                                  ...
                                  {(provided, snapshot) => (
                                    <div
                                      ...
                                      className={`space-y-1 min-h-[48px] rounded-md transition-colors p-0.5 ${
                                        snapshot.isDraggingOver ? "bg-blue-100/60 ring-1 ring-blue-300" : ""
                                      }`}
                                    >
                                      {jobs.map((wo, index) => (
                                        <Draggable key={wo.id} draggableId={wo.id} index={index}>
                                          {(prov, snap) => <JobCard wo={wo} prov={prov} snap={snap} />}
                                        </Draggable>
                                      ))}
...
// ── Month View ───────────────────────────────────────────────────────────────
function MonthView({ monthStart, workOrders, filteredTechIds, onWeekClick }) {
    ...
    <Card className="overflow-hidden border-slate-200/80">
      <div className="overflow-x-auto">
        <table className="w-full min-w-[600px]">
          <thead>
            <tr className="bg-slate-50/80">
              <th className="text-xs font-semibold text-slate-500 p-3 border-b border-r border-slate-200 w-16 text-center">Week</th>
              {daysOfWeek.map(d => (
                <th key={d} className="text-center text-xs font-semibold text-slate-500 p-3 border-b border-r border-slate-200">{d}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {weeks.map(weekStart => {
              ...
              return (
                <tr key={weekStart.format("YYYY-MM-DD")} className="border-b border-slate-100">
                  ...
                  {days.map(day => {
                    ...
                    const count = workOrders.filter(wo => {
                      const techMatch = filteredTechIds.length === 0 || filteredTechIds.includes(wo.assigned_technician_id);
...
export default function Schedule() {
  ...
  const { data: teamRoles = [] } = useQuery({
    queryKey: ["teamRoles"],
    queryFn: () => base44.entities.TeamRole.list(),
  });

  const activeTechs = useMemo(() => technicians.filter(t => t.status === "active"), [technicians]);

  // Unique roles present among active technicians
  const availableRoles = useMemo(() => {
    const roleCodes = [...new Set(activeTechs.map(t => t.team_role).filter(Boolean))];
  ...
  const toggleRole = (code) => {
    setSelectedRoles(prev => prev.includes(code) ? prev.filter(x => x !== code) : [...prev, code]);
  };

  // Technicians filtered by selected roles
  const filteredTechs = useMemo(() =>
    selectedRoles.length === 0 ? activeTechs : activeTechs.filter(t => selectedRoles.includes(t.team_role)),
  ...
  // Group filtered techs by role for week view
  ...
  // For month view: all filtered tech IDs
  const filteredTechIds = useMemo(() => filteredTechs.map(t => t.id), [filteredTechs]);

  const isToday = (day) => day.isSame(moment(), "day");

  const handleWeekClick = (offset) => {
    setWeekOffset(offset);
    setViewMode("week");
  };

  if (woLoading) {
    return <div className="p-6"><Skeleton className="h-[600px] rounded-xl" /></div>;
  }
    ...
    <div className="p-4 lg:p-6 max-w-[1600px] mx-auto space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3 justify-between">
        <div className="flex items-center gap-2">
          ...
          <Button variant="ghost" size="sm" onClick={() => { setWeekOffset(0); setMonthOffset(0); }}>
        ...
        <div className="flex items-center gap-1 bg-slate-100 rounded-lg p-1">
          <Button size="sm" variant={viewMode === "week" ? "default" : "ghost"} className="h-7 px-3 text-xs" onClick={() => setViewMode("week")}>
            <CalendarDays className="w-3.5 h-3.5 mr-1" /> Week
          </Button>
          <Button size="sm" variant={viewMode === "month" ? "default" : "ghost"} className="h-7 px-3 text-xs" onClick={() => setViewMode("month")}>
      ...
      {viewMode === "week" ? (
        <>
          <p className="text-xs text-slate-400 italic">ลาก job card เพื่อย้ายช่าง · คลิก Role header เพื่อ collapse</p>
          <WeekView
        ...
        <>
          <p className="text-xs text-slate-400 italic">คลิกตัวเลขในช่องเพื่อดูรายละเอียดในมุมมองสัปดาห์</p>
          <MonthView
