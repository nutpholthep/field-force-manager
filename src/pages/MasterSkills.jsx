...
export default function MasterSkills() {
  ...
  const handleImport = async (records) => {
    for (const r of records) await base44.entities.Skill.create(r);
    queryClient.invalidateQueries({ queryKey: ["skills"] });
    ...
    <div className="p-4 lg:p-6 max-w-[1400px] mx-auto space-y-4">
      <div className="flex flex-wrap gap-3 items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">{filtered.length} / {skills.length} skills</p>
        </div>
        <div className="flex flex-wrap gap-2 items-center">
          <MasterToolbar search={search} onSearch={setSearch} exportData={filtered} onImport={handleImport} />
          {categories.length > 0 && (
            <select value={catFilter} onChange={e => setCatFilter(e.target.value)} className="h-9 text-sm border border-slate-200 rounded-md px-2 bg-white">
