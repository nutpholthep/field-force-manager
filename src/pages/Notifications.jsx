        ...
        <div className="space-y-2">
          {notifications.map(n => {
            ...
            return (
              <Card
                key={n.id}
                className={`p-4 bg-white border-slate-200/80 transition-all ${!n.is_read ? "border-l-4 border-l-blue-500" : ""}`}
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-slate-400 mt-1">{moment(n.created_date).fromNow()}</p>
                  </div>
                  {!n.is_read && (
                    <Button variant="ghost" size="icon" className="h-7 w-7 shrink-0" onClick={() => markAsRead(n.id)}>
