...
export default function Customers() {
  ...
  const [form, setForm] = useState({ name: "", contact_person: "", email: "", phone: "", address: "", type: "commercial" });
  ...
  const filtered = customers.filter(c => {
    const matchSearch = !search || c.name?.toLowerCase().includes(search.toLowerCase()) || c.contact_person?.toLowerCase().includes(search.toLowerCase()) || c.email?.toLowerCase().includes(search.toLowerCase());
    const matchType = typeFilter === "all" || c.type === typeFilter;
    return matchSearch && matchType;
  });

  const openEdit = (c) => { setEditData(c); setForm(c); setShowForm(true); };
  const openCreate = () => { setEditData(null); setForm({ name: "", contact_person: "", email: "", phone: "", address: "", type: "commercial" }); setShowForm(true); };

  const handleSave = async () => {
    setSaving(true);
    if (editData?.id) await base44.entities.Customer.update(editData.id, form);
  ...
  const handleImport = async (records) => {
    for (const r of records) await base44.entities.Customer.create(r);
    queryClient.invalidateQueries({ queryKey: ["customers"] });
  };
    ...
    <div className="p-4 lg:p-6 max-w-[1600px] mx-auto space-y-4">
      <div className="flex flex-wrap gap-3 items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">{filtered.length} / {customers.length} customers</p>
        </div>
          ...
          <Table>
            ...
            <TableBody>
              {isLoading ? (
                ...
                <TableRow><TableCell colSpan={6} className="text-center text-slate-400 py-12">No customers found</TableCell></TableRow>
              ) : filtered.map(c => (
                <TableRow key={c.id} className="hover:bg-slate-50/50">
                  <TableCell className="font-medium text-sm text-slate-800">{c.name}</TableCell>
                  <TableCell><Badge className={`text-[10px] ${typeColors[c.type] || typeColors.commercial}`}>{c.type}</Badge></TableCell>
                  <TableCell className="text-sm text-slate-600">{c.contact_person || "—"}</TableCell>
                  ...
                  <TableCell>
                    <div className="flex gap-1">
                      ...
                      <Button variant="ghost" size="icon" className="h-7 w-7 text-red-400" onClick={() => handleDelete(c.id)}><Trash2 className="w-3 h-3" /></Button>
                    </div>
                  </TableCell>
                </TableRow>
              ))}
      ...
      <Dialog open={showForm} onOpenChange={() => setShowForm(false)}>
        <DialogContent className="max-w-md">
          <DialogHeader><DialogTitle>{editData ? "Edit Customer" : "New Customer"}</DialogTitle></DialogHeader>
          <div className="space-y-4">
            ...
            <div>
              <Label>Type</Label>
              <Select value={form.type || "commercial"} onValueChange={v => setForm(p => ({ ...p, type: v }))}>
