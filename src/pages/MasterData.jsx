...
import { Plus, Pencil, Trash2, Loader2, Wrench, Zap, CheckCircle, Flag, ExternalLink, X, Package } from "lucide-react";
...
// ServiceType Section
// ────────────────────────────────────────────────────────────
function ServiceTypeSection({ onGoToPriority }) {
  const queryClient = useQueryClient();
  const [showForm, setShowForm] = useState(false);
  const [editData, setEditData] = useState(null);
  const [saving, setSaving] = useState(false);
        ...
        <Table>
          ...
          <TableBody>
            {isLoading ? (
              ...
              <TableRow><TableCell colSpan={7} className="text-center text-slate-400 py-8">No service types yet</TableCell></TableRow>
            ) : serviceTypes.map(st => (
              <TableRow key={st.id} className="hover:bg-slate-50/50">
                ...
                <TableCell>
                  <div className="flex gap-1">
                    ...
                    <Button variant="ghost" size="icon" className="h-7 w-7 text-red-400 hover:text-red-600" onClick={() => handleDelete(st.id)}><Trash2 className="w-3 h-3" /></Button>
                  </div>
                </TableCell>
              </TableRow>
            ))}
      ...
      <Dialog open={showForm} onOpenChange={() => setShowForm(false)}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            ...
            {/* Allowed Priorities — row-based */}
            <div>
              <div className="flex items-center justify-between mb-2">
                <Label>Allowed Priorities</Label>
                <button onClick={onGoToPriority} className="flex items-center gap-1 text-xs text-blue-500 hover:text-blue-700 underline">
              ...
              {priorities.length === 0 ? (
                <div className="text-xs text-slate-400 border border-dashed border-slate-200 rounded-lg p-3 text-center">
                  ...
                  <button onClick={onGoToPriority} className="text-blue-500 underline">Add priorities first</button>
                </div>
              ) : (
...
// ────────────────────────────────────────────────────────────
const CAT_COLORS = ["#3b82f6","#10b981","#f59e0b","#ef4444","#8b5cf6","#06b6d4","#f97316","#64748b"];
...
function MaterialCategorySection() {
  ...
  const [editData, setEditData] = useState(null);
  const [saving, setSaving] = useState(false);
  const [form, setForm] = useState({ name: "", code: "", description: "", color: "#3b82f6", is_active: true });
  ...
  const openCreate = () => { setEditData(null); setForm({ name: "", code: "", description: "", color: "#3b82f6", is_active: true }); setShowForm(true); };
  const openEdit = (c) => { setEditData(c); setForm({ name: c.name, code: c.code, description: c.description || "", color: c.color || "#3b82f6", is_active: c.is_active !== false }); setShowForm(true); };

  const handleSave = async () => {
    if (!form.name || !form.code) return toast.error("Name and Code are required");
    setSaving(true);
    if (editData?.id) {
      await base44.entities.MaterialCategory.update(editData.id, form);
    } else {
      await base44.entities.MaterialCategory.create(form);
    }
    setSaving(false);
    setShowForm(false);
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        ...
        <Button size="sm" onClick={openCreate}><Plus className="w-3.5 h-3.5 mr-1" /> เพิ่มหมวดหมู่</Button>
      </div>
      <Card className="overflow-hidden border-slate-200/80">
        <Table>
          <TableHeader>
          ...
          <TableBody>
            {isLoading ? (
              ...
              <TableRow><TableCell colSpan={5} className="text-center text-slate-400 py-8">ยังไม่มีหมวดหมู่ — เพิ่มหมวดหมู่ Material</TableCell></TableRow>
            ) : categories.map(c => (
              <TableRow key={c.id} className="hover:bg-slate-50/50">
                ...
                <TableCell><code className="text-xs bg-slate-100 px-1.5 py-0.5 rounded font-mono">{c.code}</code></TableCell>
                <TableCell className="text-xs text-slate-500">{c.description || "—"}</TableCell>
                <TableCell>
                  {c.is_active !== false
                    ? <Badge className="text-[10px] bg-emerald-100 text-emerald-700">Active</Badge>
                    : <Badge variant="outline" className="text-[10px] text-slate-400">Inactive</Badge>}
                ...
                <TableCell>
                  <div className="flex gap-1">
                    ...
                    <Button variant="ghost" size="icon" className="h-7 w-7 text-red-400 hover:text-red-600" onClick={() => handleDelete(c.id)}><Trash2 className="w-3 h-3" /></Button>
                  </div>
                </TableCell>
              </TableRow>
            ))}
      ...
      <Dialog open={showForm} onOpenChange={() => setShowForm(false)}>
        <DialogContent className="max-w-md">
          <DialogHeader><DialogTitle>{editData ? "แก้ไขหมวดหมู่" : "หมวดหมู่ใหม่"}</DialogTitle></DialogHeader>
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              <div><Label>ชื่อหมวดหมู่ *</Label><Input value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} placeholder="เช่น อะไหล่ไฟฟ้า" /></div>
              <div><Label>Code *</Label><Input value={form.code} onChange={e => setForm(p => ({ ...p, code: e.target.value.toLowerCase().replace(/\s+/g, "_") }))} placeholder="เช่น electrical" /></div>
            </div>
  ...
  return (
    <div className="p-4 lg:p-6 max-w-[1400px] mx-auto space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-0.5">Manage system-wide options: priorities, service types, skills, and material categories</p>
      </div>
