...
export default function MasterMaterialCategories() {
  ...
  const handleImport = async (records) => {
    for (const r of records) await base44.entities.MaterialCategory.create(r);
    queryClient.invalidateQueries({ queryKey: ["materialCategories"] });
  ...
  const filtered = categories.filter(c => {
    ...
    const matchStatus = statusFilter === "all" || (statusFilter === "active" ? c.is_active !== false : c.is_active === false);
    ...
    <div className="p-4 lg:p-6 max-w-[1400px] mx-auto space-y-4">
      <div className="flex flex-wrap gap-3 items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">{filtered.length} / {categories.length} หมวดหมู่</p>
        </div>
        ...
        <Table>
          ...
          <TableBody>
            {isLoading ? (
              ...
              <TableRow><TableCell colSpan={5} className="text-center text-slate-400 py-8">ยังไม่มีหมวดหมู่</TableCell></TableRow>
            ) : filtered.map(c => (
