...
/**
 * Task-type specific config panel rendered below each TaskRow
 * Reads/writes task.config = { ...fields }
 */
export default function TaskConfigPanel({ task, onUpdate }) {
  const [showConfig, setShowConfig] = useState(false);
  const config = task.config || {};

  const set = (key, val) => onUpdate({ ...task, config: { ...config, [key]: val } });
          ...
          {task.type === "photo" && (
            <div className="flex items-center gap-2">
              ...
              <select
                ...
                onChange={e => set("photo_source", e.target.value)}
                className="h-6 text-xs border border-slate-200 rounded px-1.5 bg-white"
              >
...
function FileTypeInput({ config, set }) {
...
function CheckboxOptionsConfig({ config, set }) {
  const [input, setInput] = useState("");
...
function MaterialSlotConfig({ config, set, categories }) {
  const slots = config.material_slots || [{ id: `slot_${Date.now()}`, category_ids: [], keywords: [], allow_custom: true }];
...
function SlotRow({ slot, idx, categories, onChange, onRemove, showRemove }) {
