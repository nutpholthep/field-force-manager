...
function CertCard({ cert, onStatusChange, onDelete }) {
...
export default function SkillCertPanel({ technicianId, technicianName }) {
  const qc = useQueryClient();
  const fileRef = useRef();
  const [adding, setAdding] = useState(false);
  const [showBulk, setShowBulk] = useState(false);
  const [uploading, setUploading] = useState(false);
  const [newCert, setNewCert] = useState({ skill_id: "", skill_name: "", issued_date: "", expiry_date: "", cert_file_url: "", cert_file_name: "" });
  ...
  const handleAdd = async () => {
    ...
    setAdding(false);
    refresh();
    // Sync approved skills back to Technician
    syncApprovedSkills();
  };

  const syncApprovedSkills = async () => {
    const allCerts = await base44.entities.MemberSkillCert.filter({ technician_id: technicianId });
    const approved = allCerts.filter(c => c.status === "approved").map(c => c.skill_name);
    await base44.entities.Technician.update(technicianId, { skills: approved });
    qc.invalidateQueries({ queryKey: ["technicians"] });
  };

  const handleStatusChange = async (certId, status) => {
    await base44.entities.MemberSkillCert.update(certId, { status });
    refresh();
    syncApprovedSkills();
  };

  const handleDelete = async (certId) => {
    await base44.entities.MemberSkillCert.delete(certId);
    refresh();
    syncApprovedSkills();
  };
            ...
            <Select value={newCert.skill_id} onValueChange={handleSkillChange}>
              ...
              <SelectContent>
                {availableSkills.length === 0
                  ? <SelectItem value={null} disabled>All skills already added</SelectItem>
