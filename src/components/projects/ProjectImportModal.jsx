...
// Parse Excel using SheetJS (xlsx) via CDN-free approach using base44 ExtractDataFromUploadedFile
const EXPECTED_COLUMNS = ["site_name", "workflow_name", "start_date"];
...
export default function ProjectImportModal({ onClose, onDone, workflows, sites }) {
  ...
  const handleFile = async (file) => {
    ...
    // Validate each row
    const validated = parsedRows.map((row, i) => {
  ...
  const handleImport = async () => {
    ...
    for (let i = 0; i < validRows.length; i++) {
      ...
      try {
        ...
        const wf = row._wf;
        const serviceNodes = (wf?.nodes || []).filter(n => n.type === "service");
                  ...
                  {rows.map((row, i) => (
                    <tr key={i} className={row._errors.length > 0 ? "bg-red-50" : "bg-white"}>
                      ...
                      <td className="px-3 py-2 text-slate-500">
                        {row._wf ? `${(row._wf.nodes || []).filter(n => n.type === "service").length} steps` : "—"}
                      </td>
