...
export interface StuckConfirmPayload {
...
interface Props {
  open: boolean;
  onClose: () => void;
  onConfirm: (payload: StuckConfirmPayload) => void;
  saving?: boolean;
}

export default function StuckDialog({ open, onClose, onConfirm, saving }: Props) {
  ...
  const { data: reasons = [] } = useQuery<StuckReason[]>({
    queryKey: ['stuckReasons'],
    queryFn: () => entities.StuckReason.filter({ is_active: true }, 'name', 200),
    enabled: open,
  });
  ...
  const grouped = useMemo(() => filtered.reduce<Record<string, StuckReason[]>>((acc, r) => {
...
function ReasonButton({ r, selected, onClick }: ReasonButtonProps) {
  return (
    <button
      type="button"
      onClick={onClick}
