...
import type { TeamRole } from '@ffm/shared';
...
export default function TeamRoleManager({ open, onClose }: Props) {
  const qc = useQueryClient();
  const [saving, setSaving] = useState(false);
  const [newRole, setNewRole] = useState<NewRoleState>({ name: '', code: '', color: DEFAULT_COLORS[0], description: '' });
  ...
  const handleAdd = async () => {
    ...
    setSaving(true);
    await entities.TeamRole.create({ ...newRole, is_active: true } as Partial<TeamRole>);
  ...
  const handleDelete = async (id: string) => {
    await entities.TeamRole.delete(id);
    qc.invalidateQueries({ queryKey: ['teamRoles'] });
  };

  const handleToggle = async (role: TeamRole) => {
    await entities.TeamRole.update(role.id, { is_active: !role.is_active });
    qc.invalidateQueries({ queryKey: ['teamRoles'] });
  };

  const autoCode = (name: string) => name.toLowerCase().replace(/\s+/g, '_').replace(/[^a-z0-9_]/g, '');
