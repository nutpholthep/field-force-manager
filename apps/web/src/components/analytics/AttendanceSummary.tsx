...
import { LogIn, LogOut, UserX, Users, Clock } from "lucide-react";
import { toast } from "sonner";
import moment from "moment";
import type { Technician, TechnicianAttendance, WorkOrder } from "@ffm/shared";
...
export default function AttendanceSummary() {
  ...
  const { data: technicians = [] } = useQuery<Technician[]>({
    ...
    queryFn: () => entities.Technician.filter({ status: "active" }, "full_name", 200),
  });

  const { data: attendance = [] } = useQuery<TechnicianAttendance[]>({
    queryKey: ["attendance", TODAY],
    queryFn: () => entities.TechnicianAttendance.filter({ date: TODAY }, "technician_name", 200),
  ...
  const scheduledTodayIds = useMemo(() => {
    ...
    workOrders
      ...
      .forEach(wo => { if (wo.assigned_technician_id) ids.add(wo.assigned_technician_id); });
    return ids;
  }, [workOrders]);
  ...
  const ensureRecord = async (tech: Technician) => {
    const existing = attendance.find(a => a.technician_id === tech.id);
    if (existing) return existing;
    const created = await entities.TechnicianAttendance.create({
  ...
  const handleCheckIn = async (tech: Technician) => {
    const rec = await ensureRecord(tech);
    const now = moment();
    await entities.TechnicianAttendance.update(rec.id, {
      status: "checked_in",
  ...
  const handleCheckOut = async (tech: Technician) => {
    const rec = attendance.find(a => a.technician_id === tech.id);
    if (!rec) return;
    const now = moment();
  ...
  const handleMarkAbsent = async (tech: Technician) => {
    const rec = await ensureRecord(tech);
    await entities.TechnicianAttendance.update(rec.id, { status: "absent" });
    queryClient.invalidateQueries({ queryKey: ["attendance", TODAY] });
