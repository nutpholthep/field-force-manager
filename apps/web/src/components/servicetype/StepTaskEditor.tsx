...
const TASK_TYPES: Array<{ type: string; icon: LucideIcon; label: string; color: string }> = [
...
function StepCard({ step, index, onUpdate, onDelete }: StepCardProps) {
  ...
  const addTask = () => {
    const task: Task = { id: `task_${Date.now()}`, label: '', type: 'text', required: false };
...
interface Props {
  ...
  onAIExtras?: (extras: AIExtras) => void;
}

export default function StepTaskEditor({ steps = [], onChange, onAIExtras }: Props) {
  // AI generator returns looser step shape; cast to our Step.
  const handleAIApply = (newSteps: unknown, mode: 'append' | 'replace', extras: AIExtras) => {
    const typed = newSteps as Step[];
    if (mode === 'replace') onChange(typed);
    else onChange([...steps, ...typed]);
    if (extras && onAIExtras) onAIExtras(extras);
  };

  const addStep = () => {
    const step: Step = { id: `step_${Date.now()}`, name: '', optional: false, tasks: [] };
    onChange([...steps, step]);
  };

  const updateStep = (stepId: string, updated: Step) => {
