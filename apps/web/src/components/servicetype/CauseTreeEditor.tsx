...
interface CauseNodeData {
  id: string;
  label: string;
  children?: CauseNodeData[];
}

interface CauseNodeProps {
  node: CauseNodeData;
  depth: number;
  onChange: (updated: CauseNodeData) => void;
  onDelete: () => void;
  maxDepth?: number;
}

function CauseNode({ node, depth, onChange, onDelete, maxDepth = 3 }: CauseNodeProps) {
  const [expanded, setExpanded] = useState(true);
  const hasChildren = !!(node.children && node.children.length > 0);
  const canAddChild = depth < maxDepth;

  const updateLabel = (label: string) => onChange({ ...node, label });

  const addChild = () => {
    const newChild: CauseNodeData = { id: crypto.randomUUID(), label: '', children: [] };
    onChange({ ...node, children: [...(node.children || []), newChild] });
  };

  const updateChild = (idx: number, updated: CauseNodeData) => {
...
interface Props {
  causes?: CauseNodeData[];
  onChange: (causes: CauseNodeData[]) => void;
}

export default function CauseTreeEditor({ causes = [], onChange }: Props) {
  const addRoot = () => {
    const newNode: CauseNodeData = { id: crypto.randomUUID(), label: '', children: [] };
    onChange([...causes, newNode]);
  };

  const updateRoot = (idx: number, updated: CauseNodeData) => {
    const next = [...causes];
    next[idx] = updated;
    onChange(next);
  };
