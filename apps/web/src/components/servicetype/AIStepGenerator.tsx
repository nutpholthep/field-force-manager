...
function PreviewStep({ step, index }: PreviewStepProps) {
...
interface Props {
  onApply: (steps: StepItem[], mode: 'append' | 'replace', extras: AIExtras) => void;
}

export default function AIStepGenerator({ onApply }: Props) {
  const [open, setOpen] = useState(false);
  const [file, setFile] = useState<File | null>(null);
  const [stage, setStage] = useState<'idle' | 'uploading' | 'generating' | 'preview' | 'error'>('idle');
  const [errorMsg, setErrorMsg] = useState('');
  const [generatedSteps, setGeneratedSteps] = useState<StepItem[]>([]);
  const [, setSuggestedSkills] = useState<string[]>([]);
  const [, setSuggestedDurationHrs] = useState<number | null>(null);
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const res = await http.post<{ file_url: string }>('/uploads', formData);
      fileUrl = res.data.file_url;
    } catch (e) {
      setStage('error');
      setErrorMsg('อัพโหลดไฟล์ไม่สำเร็จ: ' + (e as Error).message);
      return;
    }
...
Return ONLY valid JSON:
{
  ...
  "duration_reason": "Brief explanation of the estimate"
    ...
    try {
      // TODO: backend AI endpoint
      const response = await http.post<{
        steps?: Array<{ name?: string; optional?: boolean; tasks?: Array<{ label?: string; type?: string; required?: boolean }> }>;
        required_skills?: string[];
      ...
      const withIds: StepItem[] = rawSteps.map((step, si) => ({
      ...
      setStage('error');
      setErrorMsg('AI ประมวลผลไม่สำเร็จ: ' + (e as Error).message);
    }
  };
  ...
  const removeSkill = (idx: number) => setEditSkills(s => s.filter((_, i) => i !== idx));
  const addSkill = () => {
