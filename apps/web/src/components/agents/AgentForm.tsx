...
type AgentFormData = Partial<AIAgent> & {
  name: string;
  description: string;
...
interface Props {
  ...
  onClose: () => void;
  onSaved: () => void;
  editData?: AIAgent | null;
}

export default function AgentForm({ open, onClose, onSaved, editData }: Props) {
  ...
  const update = <K extends keyof AgentFormData>(k: K, v: AgentFormData[K]) =>
  ...
  const handleSave = async () => {
    setSaving(true);
    if (editData?.id) {
      await entities.AIAgent.update(editData.id, form);
    } else {
      await entities.AIAgent.create(form);
    }
    setSaving(false);
    onSaved();
    onClose();
