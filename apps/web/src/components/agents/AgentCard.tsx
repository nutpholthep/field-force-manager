...
const DATA_SKILL_LABELS: Record<string, string> = {
...
export default function AgentCard({ agent, onEdit, onDelete, onRun }: Props) {
  return (
    <Card className="p-5 bg-white border-slate-200 hover:shadow-md transition-all">
      ...
      {/* Data skills */}
      {agent.data_skills && agent.data_skills.length > 0 && (
      ...
      {agent.assigned_zone_names && agent.assigned_zone_names.length > 0 && (
        <div className="flex flex-wrap gap-1 mt-2.5 pt-2.5 border-t border-slate-100">
          ...
          {agent.assigned_zone_names.map((z: string) => (
