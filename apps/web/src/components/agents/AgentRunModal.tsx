...
import { Bot, Loader2, Zap, CheckCircle2, AlertTriangle } from "lucide-react";
...
interface AgentRunResult {
  zone_health: 'Good' | 'Warning' | 'Critical';
  summary: string;
  alerts: AgentRunAlert[];
  actions_executed: number;
  actions?: AgentRunAction[];
  recommendations?: string[];
  notifications_to_create?: unknown[];
}
...
export default function AgentRunModal({ open, onClose, agent, onSaved }: Props) {
  ...
  const handleRun = async () => {
    ...
    try {
      // TODO: backend AI endpoint — replace with actual executeAIAgent invocation
      const response = await http.post<{ data: { result: { actions_executed?: number; actions?: AgentRunAction[]; llm_summary?: string; reason?: string }; error?: string } }>(
        '/ai/invoke',
        { function: 'executeAIAgent', agent_id: agent.id },
      );
      const { result: runResult, error } = response.data.data;
      ...
      if (error) {
        ...
        const actionsExecuted = runResult.actions_executed ?? 0;
        const actions = runResult.actions || [];
        const summary = runResult.llm_summary || runResult.reason || `${actionsExecuted} reassignments completed`;
        setResult({
          zone_health: actionsExecuted > 0 ? 'Good' : 'Warning',
          summary,
          alerts: actions.map((a: AgentRunAction) => ({
            type: 'reassign',
      ...
      setResult({
        zone_health: 'Critical',
        summary: 'Error executing agent: ' + (e instanceof Error ? e.message : String(e)),
        alerts: [],
        recommendations: [],
        ...
        {!result && !running && (
          <div className="space-y-3 text-sm text-slate-600">
            ...
            <div className="flex flex-wrap gap-1.5">
              {agent?.data_skills?.map((s: string) => (
                <Badge key={s} variant="secondary" className="text-xs">{s.replace("_", " ")}</Badge>
              ))}
            </div>
            {agent?.assigned_zone_names && agent.assigned_zone_names.length > 0 && (
