...
import { Play, Square, GitBranch, ZoomIn, ZoomOut, Plus, Loader2 } from 'lucide-react';
import NodeConfigPanel from './NodeConfigPanel';
import StepTaskEditor from '@/components/servicetype/StepTaskEditor';
import { useQueryClient } from '@tanstack/react-query';
import { entities } from '@/lib/entity-client';
import type { ServiceType, Technician, Zone } from '@ffm/shared';

const NODE_WIDTH = 160;
const NODE_HEIGHT = 64;
const MIN_ZOOM_W = 700;
const MAX_ZOOM_W = 2000;

interface WorkflowNodeShape {
  id: string;
  type: 'start' | 'end' | 'service';
  label: string;
  x: number;
  y: number;
...
interface NodeBoxProps {
  ...
  onPortMouseDown: (e: React.MouseEvent, id: string, portType: 'in' | 'out') => void;
  onDelete: (id: string) => void;
  connecting: boolean;
}

function NodeBox({ node, selected, onSelect, onDragStart, onPortMouseDown, onDelete, connecting }: NodeBoxProps) {
...
function NewServiceTypeDialog({ onClose, onCreated }: NewServiceTypeDialogProps) {
  ...
  const handleSave = async () => {
    if (!name.trim()) return;
    setSaving(true);
    const created = await entities.ServiceType.create({
      name: name.trim(),
...
interface Props {
  ...
  zones: Zone[];
  onChange: (data: { nodes: WorkflowNodeShape[]; edges: WorkflowEdgeShape[] }) => void;
}

export default function WorkflowCanvas({ workflow, serviceTypes, technicians, zones, onChange }: Props) {
  const svgRef = useRef<SVGSVGElement | null>(null);
  const qc = useQueryClient();
  const [nodes, setNodes] = useState<WorkflowNodeShape[]>(workflow?.nodes || []);
  const [edges, setEdges] = useState<WorkflowEdgeShape[]>(workflow?.edges || []);
  const [selected, setSelected] = useState<string | null>(null);
  const [dragging, setDragging] = useState<{ id: string; offX: number; offY: number } | null>(null);
  const [connecting, setConnecting] = useState<{ fromId: string; fromX: number; fromY: number; curX: number; curY: number } | null>(null);
  const [configNode, setConfigNode] = useState<WorkflowNodeShape | null>(null);
  const [viewBox, setViewBox] = useState<ViewBox>({ x: -40, y: -40, w: 1000, h: 700 });
  const [panning, setPanning] = useState<{ startX: number; startY: number; vb: ViewBox } | null>(null);
  ...
  const save = useCallback((ns: WorkflowNodeShape[], es: WorkflowEdgeShape[]) => {
    onChange({ nodes: ns, edges: es });
  }, [onChange]);

  const addNode = (type: 'start' | 'end' | 'service', serviceType: ServiceType | null = null) => {
  ...
  const onSvgMouseDown = (e: React.MouseEvent<SVGSVGElement>) => {
    ...
    const isBackground = target === svgRef.current || target.tagName === 'svg' || (target.tagName === 'rect' && target.closest('g') === null);
  ...
  const onNodeDragStart = (e: React.MouseEvent, id: string) => {
  ...
  const onPortMouseDown = (e: React.MouseEvent, nodeId: string, portType: 'in' | 'out') => {
    e.preventDefault();
  ...
  const onSvgMouseMove = useCallback((e: React.MouseEvent<SVGSVGElement>) => {
    ...
    if (connecting) {
      setConnecting(c => c ? ({ ...c, curX: svgP.x, curY: svgP.y }) : c);
    }
    if (panning) {
  ...
  const edgePath = (edge: WorkflowEdgeShape) => {
  ...
  const addServiceNodeAtEnd = (serviceType: ServiceType) => {
    ...
    let newNodes: WorkflowNodeShape[] = [...nodes, newNode];
    if (endNode) {
    ...
    <div className="flex h-full min-h-0">
        ...
        >
          ...
          {nodes.map(node => (
            <NodeBox key={node.id} node={node} selected={selected === node.id}
              onSelect={(id) => { setSelected(id); setConfigNode(nodes.find(n => n.id === id) || null); }}
      ...
      {configNode && configNode.type === 'service' && (
        <NodeConfigPanel
          ...
          onChange={(updated) => {
            const ns = nodes.map(n => n.id === updated.id ? updated as WorkflowNodeShape : n);
            setNodes(ns);
            setConfigNode(updated as WorkflowNodeShape);
            save(ns, edges);
          }}
        />
      )}
