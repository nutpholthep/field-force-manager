...
import type { Skill } from '@ffm/shared';
...
export default function ConfigSkills() {
  ...
  const [form, setForm] = useState<SkillForm>({ name: '', description: '', category: '' });
  ...
  const openEdit = (s: Skill) => { setEditData(s); setForm({ name: s.name, description: s.description || '', category: s.category || '' }); setShowForm(true); };

  const handleSave = async () => {
    setSaving(true);
    if (editData?.id) {
      await entities.Skill.update(editData.id, form as Partial<Skill>);
    } else {
      await entities.Skill.create(form as Partial<Skill>);
    }
    setSaving(false);
    setShowForm(false);
