...
import type { ServiceType, Workflow, Skill, PriorityMaster } from '@ffm/shared';
...
export default function ConfigServiceTypes() {
  ...
  const [stepsView, setStepsView] = useState<ServiceType | null>(null);
  const [causesView, setCausesView] = useState<ServiceType | null>(null);
  const [deleteBlock, setDeleteBlock] = useState<DeleteBlock | null>(null);
  const [form, setForm] = useState<ServiceTypeForm>({
  ...
  const { data: workflows = [] } = useQuery<Workflow[]>({
    queryKey: ['workflows'],
    queryFn: () => entities.Workflow.list('name', 200),
  });
  ...
  const priorityMap: Record<string, PriorityMaster> = Object.fromEntries(priorities.map((p) => [p.id, p]));
  ...
  const handleSave = async () => {
    setSaving(true);
    if (editData?.id) {
      await entities.ServiceType.update(editData.id, form as Partial<ServiceType>);
    } else {
      await entities.ServiceType.create(form as Partial<ServiceType>);
    }
    setSaving(false);
    setShowForm(false);
  ...
  const handleDelete = async (st: ServiceType) => {
    ...
    await entities.ServiceType.delete(st.id);
    queryClient.invalidateQueries({ queryKey: ['serviceTypes'] });
  ...
  const handleStepsChange = async (steps: unknown[]) => {
    if (!stepsView?.id) return;
    setStepsView((prev) => (prev ? { ...prev, steps } as ServiceType : prev));
    await entities.ServiceType.update(stepsView.id, { steps } as Partial<ServiceType>);
    queryClient.invalidateQueries({ queryKey: ['serviceTypes'] });
  };

  const handleAIExtras = async (extras: { default_duration_hrs?: number; required_skills?: string[] }) => {
    if (!stepsView?.id) return;
    const updates: Record<string, unknown> = {};
    if (extras.default_duration_hrs) updates.default_duration_hrs = extras.default_duration_hrs;
    if (extras.required_skills && extras.required_skills.length > 0) {
    ...
    setStepsView((prev) => (prev ? { ...prev, ...updates } as ServiceType : prev));
    await entities.ServiceType.update(stepsView.id, updates as Partial<ServiceType>);
    queryClient.invalidateQueries({ queryKey: ['serviceTypes'] });
    toast.success('AI อัปเดต SLA/Skills แล้ว');
  };

  const handleCausesChange = async (causes: unknown[]) => {
    if (!causesView?.id) return;
    setCausesView((prev) => (prev ? { ...prev, causes } as ServiceType : prev));
    await entities.ServiceType.update(causesView.id, { causes } as Partial<ServiceType>);
    queryClient.invalidateQueries({ queryKey: ['serviceTypes'] });
  };
          ...
          <div className="space-y-4">
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <Link href="/config/priority" className="flex items-center gap-1 text-xs text-blue-500 hover:text-blue-700 underline">
              ...
              {priorities.length === 0 ? (
                <div className="text-xs text-slate-400 border border-dashed border-slate-200 rounded-lg p-3 text-center">
                  ...
                  <Link href="/config/priority" className="text-blue-500 underline">Add priorities first</Link>
                </div>
              ) : (
