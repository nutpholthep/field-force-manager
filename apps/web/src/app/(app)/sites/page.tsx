...
import type { Customer, Site, Zone } from '@ffm/shared';
...
const emptyForm: SiteFormState = {
  ...
  address: '',
  zone_name: '',
  site_type: 'office' as Site['site_type'],
  equipment: [],
  access_instructions: '',
  status: 'active',
  latitude: null,
...
export default function SitesPage() {
  ...
  const { data: sites = [], isLoading } = useQuery<Site[]>({
    queryKey: ['sites'],
    queryFn: () => entities.Site.list('-created_date', 200),
  });
  ...
  const handleSave = async () => {
    setSaving(true);
    if (editData?.id) await entities.Site.update(editData.id, form);
    else await entities.Site.create(form);
    setSaving(false);
    setShowForm(false);
  ...
  const handleDelete = async (id: string) => {
    await entities.Site.delete(id);
    queryClient.invalidateQueries({ queryKey: ['sites'] });
  };

  const handleImport = async (records: Partial<Site>[]) => {
    for (const r of records) await entities.Site.create(r);
    queryClient.invalidateQueries({ queryKey: ['sites'] });
  };

  const handleCoordConfirm = (lat: number, lng: number) => {
    ...
    <div className="p-4 lg:p-6 max-w-[1600px] mx-auto space-y-4">
      <div className="flex flex-wrap gap-3 items-center justify-between">
        ...
        <div className="flex flex-wrap gap-2 items-center">
          <MasterToolbar
            ...
            onFilter={setSiteStatusFilter}
            exportData={filtered as unknown as Record<string, unknown>[]}
            onImport={(records) => handleImport(records as unknown as Partial<Site>[])}
          />
          <select
            value={typeFilter}
          ...
          >
            ...
            {Object.keys(siteTypeIcons).map((t) => (
      ...
      {isLoading ? (
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((s) => (
            <Card
              key={s.id}
              className="p-4 bg-white border-slate-200/80 hover:shadow-md transition-shadow"
            >
      ...
      <Dialog open={showForm} onOpenChange={() => setShowForm(false)}>
        <DialogContent className="max-w-md max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editData ? 'Edit Site' : 'New Site'}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <Label>Type</Label>
                <Select
                  value={form.site_type || 'office'}
                  onValueChange={(v) =>
                    setForm((p) => ({ ...p, site_type: v as Site['site_type'] }))
                  }
                >
                  <SelectTrigger>
                    <SelectValue />
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(v) => setForm((p) => ({ ...p, zone_name: v }))}
                >
                  <SelectTrigger>
                    <SelectValue placeholder="Select..." />
                  </SelectTrigger>
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) =>
                  setForm((p) => ({ ...p, access_instructions: e.target.value }))
                }
              />
            </div>
