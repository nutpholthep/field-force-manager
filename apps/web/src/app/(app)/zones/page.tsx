...
import type { AIAgent, Technician, WorkOrder, Zone } from '@ffm/shared';

const ZoneMap = dynamic(() => import('@/components/zones/ZoneMap'), { ssr: false });
...
export default function ZonesPage() {
  ...
  const { data: zones = [], isLoading } = useQuery<Zone[]>({
    queryKey: ['zones'],
    queryFn: () => entities.Zone.list('-created_date', 100),
  });

  const { data: technicians = [] } = useQuery<Technician[]>({
    queryKey: ['technicians'],
    queryFn: () => entities.Technician.list('-created_date', 500),
  });

  const { data: workOrders = [] } = useQuery<WorkOrder[]>({
    queryKey: ['workOrders'],
    queryFn: () => entities.WorkOrder.list('-created_date', 500),
  });

  const { data: agents = [] } = useQuery<AIAgent[]>({
    queryKey: ['agents'],
    queryFn: () => entities.AIAgent.list('name', 100),
  });
  ...
  const openEdit = (zone: Zone) => {
    ...
    setForm({
      ...
      provinces: (zone as unknown as { provinces?: string[] }).provinces || [],
    ...
    try {
      if (form.agent_id) {
        ...
        if (agent) {
          ...
          await entities.AIAgent.update(form.agent_id, {
            assigned_zone_ids: newIds,
            assigned_zone_names: newNames,
          });
        }
      }
      if (editData?.id) {
        await entities.Zone.update(editData.id, form);
        toast.success('Zone updated');
      } else {
        await entities.Zone.create(form);
  ...
  const removeProvince = (p: string) =>
    setForm((prev) => ({ ...prev, provinces: prev.provinces.filter((x) => x !== p) }));

  const startDrawing = (zone: Zone) => setDrawingZone(zone);

  const handleMapSave = async (latlngs: Array<[number, number]> | null) => {
    if (!drawingZone) return;
    setSaving(true);
    await entities.Zone.update(drawingZone.id, { polygon: latlngs || [] });
          ...
          <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 bg-white">
            <div>
              <h2 className="font-semibold text-slate-800">
                {editData ? 'Edit Zone' : 'New Zone'}
              </h2>
              <p className="text-xs text-slate-400 mt-0.5">
                {editData ? `Editing: ${editData.name}` : 'Create a new service zone'}
              </p>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-5">
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(v) =>
                  setForm((prev) => ({ ...prev, agent_id: v === '__none__' ? '' : v }))
                }
              >
              ...
              {agents.length === 0 && (
                <p className="text-[11px] text-slate-400 mt-1">
                  ...
                  <a href="/agents" className="text-blue-500 underline">
                    Agents page
                  </a>
                </p>
              )}
      ...
      <div className="relative h-[40vh] lg:h-auto lg:flex-1 order-first lg:order-last shrink-0">
        {drawingZone && (
          <div className="absolute top-3 left-1/2 -translate-x-1/2 z-[1000] flex items-center gap-2 bg-white border border-slate-200 rounded-xl shadow-lg px-4 py-2.5 max-w-[90vw]">
            <div
              ...
              style={{ backgroundColor: drawingZone.color || '#3b82f6' }}
            />
            <span className="text-sm font-semibold text-slate-700 truncate">
              {drawingZone.name}
            </span>
            <span className="hidden sm:block text-xs text-slate-400 border-l border-slate-200 pl-2 ml-1">
              Use polygon tool on map
            </span>
            <Button
              size="sm"
              className="ml-2 bg-blue-600 hover:bg-blue-700 h-7 text-xs shrink-0"
              onClick={triggerMapSave}
              disabled={saving}
            >
              {saving ? (
        ...
        {!drawingZone && zones.every((z) => !z.polygon?.length) && zones.length > 0 && (
          <div className="absolute top-3 left-1/2 -translate-x-1/2 z-[1000] bg-white/90 backdrop-blur-sm border border-slate-200 rounded-lg shadow px-4 py-2 text-xs text-slate-500 whitespace-nowrap">
            Click <PenLine className="w-3 h-3 inline mx-1 text-blue-500" /> on a zone to draw its
            boundary
          </div>
        )}
        <ZoneMap
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-2">
          {isLoading ? (
            ...
            zones.map((zone) => {
              ...
              const provinces =
                (zone as unknown as { provinces?: string[] }).provinces || [];
              ...
              return (
                ...
                >
                      ...
                      >
                        <PenLine
                          className={`w-3.5 h-3.5 ${
                            isDrawing ? 'text-blue-600' : 'text-slate-400 hover:text-blue-500'
                          }`}
                        />
                      </Button>
                      <Button
                  ...
                  {zone.description && (
                    <p className="text-[11px] text-slate-500 mt-1.5 line-clamp-1 ml-5">
                      {zone.description}
                    </p>
                  )}
                  ...
                  <div className="flex items-center gap-3 mt-2.5 ml-5">
                    ...
                    {zone.polygon?.length > 0 ? (
                      ...
                      <Badge
                        variant="outline"
                        className="text-[9px] text-slate-400 ml-auto px-1.5"
                      >
                        No polygon
                      </Badge>
                    )}
                  ...
                  {provinces.length > 0 && (
                    <div className="flex flex-wrap gap-1 mt-2 ml-5">
                      {provinces.slice(0, 4).map((p: string) => (
