...
import type { Project, Workflow, WorkOrder } from '@ffm/shared';
...
function ProjectCard({ project, workflows, onSelect }: ProjectCardProps) {
  const wf = workflows.find((w) => w.id === project.workflow_id);
  const s = STATUS_CONFIG[project.status as string] || STATUS_CONFIG.planning;
    ...
    <div className="bg-white border border-slate-200 rounded-xl p-4 hover:shadow-md transition-shadow cursor-pointer" onClick={() => onSelect(project)}>
      <div className="flex items-start justify-between gap-3 mb-3">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-0.5">
            ...
            <Badge className={`text-[10px] px-1.5 py-0 border ${PRIORITY_COLOR[project.priority as string]}`}>{project.priority}</Badge>
          </div>
      ...
      <div className="flex items-center justify-between mt-1">
        ...
        <Button
          size="sm"
          variant="ghost"
          className="h-6 text-xs gap-1 text-blue-600 hover:bg-blue-50"
...
const WO_STATUS_LABEL: Record<string, string> = { created: 'Created', assigned: 'Assigned', accepted: 'Accepted', traveling: 'Traveling', on_site: 'On Site', working: 'Working', completed: 'Completed', cancelled: 'Cancelled' };
const WO_STATUS_COLOR: Record<string, string> = { created: 'bg-slate-100 text-slate-600', assigned: 'bg-blue-100 text-blue-700', accepted: 'bg-indigo-100 text-indigo-700', traveling: 'bg-yellow-100 text-yellow-700', on_site: 'bg-orange-100 text-orange-700', working: 'bg-purple-100 text-purple-700', completed: 'bg-emerald-100 text-emerald-700', cancelled: 'bg-red-100 text-red-600' };
...
function ProjectDetailModal({ project, workflows, onClose }: ProjectDetailModalProps) {
  const wf = workflows.find((w) => w.id === project.workflow_id);
  ...
  const { data: workOrders = [] } = useQuery<WorkOrder[]>({
    ...
    queryFn: () => entities.WorkOrder.filter({ project_id: project.id }),
    enabled: !!project.id,
  });
                    ...
                    <div className="mx-3 mb-3 space-y-1.5">
                      {linkedWOs.map((wo) => (
                        <div key={wo.id} className="bg-white rounded-lg border border-slate-200 p-2.5 flex items-center gap-2.5">
                          ...
                          <Badge className={`text-[10px] shrink-0 ${WO_STATUS_COLOR[wo.status as string] || 'bg-slate-100 text-slate-600'}`}>
                            {WO_STATUS_LABEL[wo.status as string] || wo.status}
                          </Badge>
                        </div>
                      ))}
                    </div>
...
export default function Projects() {
  ...
  const { data: projects = [], isLoading } = useQuery<Project[]>({ queryKey: ['projects'], queryFn: () => entities.Project.list('-created_date', 200) });
  const { data: workflows = [] } = useQuery<Workflow[]>({ queryKey: ['workflows'], queryFn: () => entities.Workflow.list('name', 100) });
