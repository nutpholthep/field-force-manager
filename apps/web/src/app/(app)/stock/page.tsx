...
import { Plus, Search, Pencil, Trash2, MoreVertical, Package, AlertTriangle, X } from 'lucide-react';
import { toast } from 'sonner';
import { entities } from '@/lib/entity-client';
import type { Material, MaterialCategory } from '@ffm/shared';
...
function MaterialForm({ open, onClose, onSaved, editData, categories }: MaterialFormProps) {
  ...
  const [form, setForm] = useState<MaterialFormData>(editData ? { ...defaultForm, ...editData } : defaultForm);
  const [saving, setSaving] = useState(false);
  const [keywordInput, setKeywordInput] = useState('');

  useEffect(() => {
    setForm(editData ? { ...defaultForm, ...editData } : defaultForm);
    setKeywordInput('');
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [editData, open]);

  const update = <K extends keyof MaterialFormData>(k: K, v: MaterialFormData[K]) => setForm((p) => ({ ...p, [k]: v }));
  ...
  const handleSave = async () => {
    if (!form.item_number || !form.item_name) {
      toast.error('รหัสและชื่อสินค้าจำเป็น');
      return;
    }
    setSaving(true);
    if (editData?.id) {
      await entities.Material.update(editData.id, form as Partial<Material>);
    } else {
      await entities.Material.create(form as Partial<Material>);
    }
    setSaving(false);
    onSaved();
...
export default function StockManagement() {
  ...
  const { data: materials = [], isLoading } = useQuery<Material[]>({
    queryKey: ['materials'],
    queryFn: () => entities.Material.list('-created_date', 500),
  });
  ...
  const handleDelete = async (id: string) => {
    await entities.Material.delete(id);
