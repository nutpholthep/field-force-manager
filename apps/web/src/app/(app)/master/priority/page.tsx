...
export default function MasterPriorityPage() {
  ...
  const handleImport = async (records: Partial<PriorityMaster>[]) => {
    for (const r of records) await entities.PriorityMaster.create(r);
    queryClient.invalidateQueries({ queryKey: ['priorityMasters'] });
      ...
      <div className="flex flex-wrap gap-3 items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">
            {filtered.length} / {priorities.length} priorities
          </p>
        </div>
        <div className="flex flex-wrap gap-2 items-center">
          <MasterToolbar
            ...
            onImport={(records) => handleImport(records as unknown as Partial<PriorityMaster>[])}
          />
