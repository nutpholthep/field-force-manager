...
import type { StuckReason } from '@ffm/shared';
...
interface StuckReasonFormState {
  name: string;
  code: string;
  category: string;
  description: string;
...
export default function MasterStuckReasonsPage() {
  const queryClient = useQueryClient();
  const [showForm, setShowForm] = useState(false);
  const [editData, setEditData] = useState<StuckReason | null>(null);
  const [saving, setSaving] = useState(false);
  const [form, setForm] = useState<StuckReasonFormState>({
    name: '',
    code: '',
    category: 'other',
    description: '',
  ...
  const { data: reasons = [], isLoading } = useQuery<StuckReason[]>({
    queryKey: ['stuckReasons'],
    queryFn: () => entities.StuckReason.list('name', 200),
  });
  ...
  const handleSave = async () => {
    setSaving(true);
    if (editData?.id) {
      await entities.StuckReason.update(editData.id, form);
    } else {
      await entities.StuckReason.create(form);
    }
    setSaving(false);
    setShowForm(false);
  ...
  const handleDelete = async (id: string) => {
    await entities.StuckReason.delete(id);
    queryClient.invalidateQueries({ queryKey: ['stuckReasons'] });
    ...
    <div className="p-4 lg:p-6 max-w-[1200px] mx-auto space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">
            Master data สำหรับสาเหตุที่งานติดขัด ใช้เลือกเมื่อเปลี่ยนสถานะเป็น Stuck
          </p>
        </div>
                  ...
                  <TableCell>
                    <span
                      className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${
                        CATEGORY_META[r.category]?.color || 'bg-slate-100 text-slate-600'
                      }`}
                    >
      ...
      <Dialog open={showForm} onOpenChange={() => setShowForm(false)}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>{editData ? 'แก้ไข Stuck Reason' : 'เพิ่ม Stuck Reason'}</DialogTitle>
          </DialogHeader>
