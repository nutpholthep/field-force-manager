...
import {
  ...
  Loader2, Check, Settings2, Lock,
} from 'lucide-react';
import {
  PAGE_LIST, ACTIONS, DEFAULT_ROLE_PERMISSIONS,
  loadPermissions, savePermissions,
} from '@/lib/permissions';
import type { PermissionsMap, Role } from '@/lib/permissions';
...
function CheckCell({ checked, locked, onChange }: CheckCellProps) {
  if (locked) {
    return (
...
export default function UserManagement() {
  ...
  const [permissions, setPermissions] = useState<PermissionsMap>(() => loadPermissions());
  ...
  const handleRoleChange = async (userId: string, newRole: string) => {
    ...
    await entities.User.update(userId, { role: newRole } as Partial<User>);
  ...
  const handleInvite = async () => {
    ...
    setInviting(true);
    try {
      // TODO: backend invite endpoint — confirm path
      await http.post('/users/invite', { email: inviteEmail.trim(), role: inviteRole });
  ...
  const togglePage = (role: Role, page: string) => {
    const updated = JSON.parse(JSON.stringify(permissions)) as PermissionsMap;
  ...
  const toggleAction = (role: Role, action: string) => {
    const updated = JSON.parse(JSON.stringify(permissions)) as PermissionsMap;
    (updated[role].actions as Record<string, boolean>)[action] = !(updated[role].actions as Record<string, boolean>)[action];
    setPermissions(updated);
    setPermDirty(true);
  };
  ...
  const handleResetPermissions = () => {
    const fresh = JSON.parse(JSON.stringify(DEFAULT_ROLE_PERMISSIONS)) as PermissionsMap;
                      ...
                      <tr key={role.value}>
                        ...
                        {ACTIONS.map((action) => (
                          <td key={action} className="py-2.5 px-3">
                            <CheckCell
                              checked={(permissions[role.value]?.actions as Record<string, boolean>)?.[action] ?? false}
